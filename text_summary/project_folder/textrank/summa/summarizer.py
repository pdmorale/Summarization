...
from textrank.summa.preprocessing.textcleaner import clean_text_by_sentences as _clean_text_by_sentences
from textrank.summa.commons import build_graph as _build_graph
...
def summarize(text, ratio=0.2, words=None, language="english", split=False, scores=False, additional_stopwords=None):
    ...
    if language == "japanese":
        return [",".join(sentence).replace(",","").replace("'","").replace(" ","") for sentence in _format_results(extracted_sentences, split, scores).split("\n")]
    else:
        return _format_results(extracted_sentences, split, scores).split("\n")
