...
INCLUDING_FILTER = ['NN', 'JJ']
EXCLUDING_FILTER = []
INCLUDING_FILTER_J = ['NO','PR','NN',"ADJ"]


def _get_pos_filters():
    return frozenset(INCLUDING_FILTER), frozenset(EXCLUDING_FILTER)

def _get_pos_filters_J():
    return frozenset(INCLUDING_FILTER_J), frozenset(EXCLUDING_FILTER)
...
def _get_words_for_graph_J(units):
    include_filters, exclude_filters = _get_pos_filters_J()
    if include_filters and exclude_filters:
        raise ValueError("Can't use both include and exclude filters, should use only one")

    result = []
    for word, unit in units.items():
...
def _get_combined_keywords_J(_keywords, split_text):
    """
    :param keywords:dict of keywords:scores
    :param split_text: list of strings
    :return: combined_keywords:list
...
def keywords(text, ratio=0.2, words=None, language="english", split=False, scores=False, deaccent=False, additional_stopwords=None):
    ...
    units = _clean_text_by_word(text, language, deacc=deaccent, additional_stopwords=additional_stopwords)
    split_text = list(_tokenize_by_word(text))

    # Creates the graph and adds the edges
    if language == "japanese":
        graph =  _build_graph(_get_words_for_graph_J(units))
    else:
        graph = _build_graph(_get_words_for_graph(units))

    _set_graph_edges(graph, units, split_text)
    ...
    lemmas_to_word = _lemmas_to_words(units)
    keywords = _get_keywords_with_score(extracted_lemmas, lemmas_to_word)
    # text.split() to keep numbers and punctuation marks, so separeted concepts are not combined
    if language == "japanese":
        # splitText = [entry.word for entry in semtence for sentences in cube(text)]
        combined_keywords = [key for key in keywords.keys()]
    else:
        combined_keywords = _get_combined_keywords(keywords, text.split())
        
    return _format_results(keywords, combined_keywords, split, scores).split("\n")
...
def get_graph(text, language="english", deaccent=False):
    ...
    if language == "japanese":
        graph = _build_graph(_get_words_for_graph_J(tokens))
    else:
