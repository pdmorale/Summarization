...
class NewsMining():
    """News Mining"""
    def __init__(self,content, filePath,filename,TranslateJ2E=False):
        self.TranslateBLN = TranslateJ2E
        ...
        self.impt_sents = summarize(self.content, words=3)[0].split("。")
        self.remove_noisy()
        self.clean_spaces()
        ...
        for i, sent in enumerate(self.doc.sents):
            ...
            if collected_ners:  # only extract self.triples when the sentence contains 'PERSON', 'ORG', 'GPE'
                ...
                self.ner_sents.append([token.text + '_' + token.label_ for token in sent_doc.ents])
        ...
        # keyGen = keyword.TextRank4Keyword()
        # self.keywords = keyGen.get_keywords()
        for keywrd in self.keywords:
            # if self.content.count(keyword) > 3 and len(keyword)>1:
            if 10 > len(keywrd) > 1:
                name = keywrd
                cate = 'keyword'
        ...
        for ImpSent in self.impt_sents:
            ...
            if len([[token.text for token in sent] for sent in nlp(ImpSent).sents][0])>3:
                    name = ImpSent.replace("\r", "")
                    cate = 'body'
                    self.events.append([name, cate])
        # 06 get NER from whole article
        self.ner_dict = {i[0]: i[1] for i in Counter(self.ners).most_common(20)}
        for ner in self.ner_dict:
            name = ner.split('_')[0]  # Jessica Miller
            cate = self.ner_dict_orig.get(ner.split('_')[1], "unknown")   # PERSON

            if len(name)>1 and cate!="unknown":
                self.events.append([name, cate])
        ...
        co_events = [[i.split('@')[0].split(
            '_')[0], i.split('@')[1].split('_')[0]] for i in co_dict]
    ...
    def collect_ners(self, ents):
        ...
        for token in ents:
            if token.label_ in self.ners_label:
                collected_ners.append(token.text + '_' + token.label_)
    ...
    #def extract_keywords(self, self.words_postags):
    #    return self.textranker.extract_keywords(self.words_postags, 10)
    ...
    def saveHTML(self):
        # 08 show event graph
        self.graph_shower = GraphShow(self.fullPath,self.events,self.TranslateBLN)
